-- 0007_repair_admin_credit_grants.sql
--
-- Standalone repair for the administrator credit grant RPC. This can be run
-- safely even when 0005 was already applied. It replaces the function,
-- reapplies service-role-only permissions, and refreshes the PostgREST schema
-- cache so /api/admin/credits can call it immediately.
...
BEGIN
    ...
    IF FOUND THEN
        ...
        available_credits := v_existing.available_balance_after;
        reserved_credits := v_existing.reserved_balance_after;
        RETURN NEXT;
        RETURN;
    END IF;
...
COMMENT ON FUNCTION admin_grant_credits(uuid, uuid, bigint, text, text)
IS 'Atomically grants user credits with an idempotent ledger entry and administrator audit log.';
